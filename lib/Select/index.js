...
class Select extends React.Component {
    ...
    handleKeyPress(e) {
        ...
        if (e.keyCode == 13) {
            if (this.props.options.length > 0)
                this.props.onSelect(this.props.options[this.state.index]);
        }
    }
    componentDidMount() {
    ...
    render() {
        return (React.createElement(Container, null,
            React.createElement(popup_1.Popup, { visible: this.state.visible, handleDismiss: () => this.setState({ visible: false }), direction: "left-bottom", width: "100%", content: React.createElement(ListContainer, { size: this.props.options.length }, this.props.options.map((item, index) => {
                    return (React.createElement(Item, { active: index == this.state.index, key: index, onClick: () => {
                            this.setState({ visible: false });
                            this.props.onSelect(item);
                        } },
                        React.createElement(ItemText, null, item.option)));
                })) },
                React.createElement(InnerContainer, null,
                    React.createElement(Text, { onClick: () => this.setState({ visible: true }) }, this.props.options[this.props.selected].option),
