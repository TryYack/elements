...
exports.AvatarComponent = (props) => {
    ...
    const [presence, setPresence] = react_1.useState("");
    const [textColor, setTextColor] = react_1.useState(props.color || props.dark ? "white" : "black");
    ...
    react_1.useEffect(() => {
        const timer = setInterval(() => {
            ...
            if (!window[PRESENCES])
                return;
            if (!window[PRESENCES][props.userId])
                return;
            if (!window[PRESENCES][props.userId].p)
                return;
    ...
    return (React.createElement(Container, { width: width, height: height, onMouseEnter: () => setOver(true), onMouseLeave: () => setOver(false) },
        (props.onEditClick && props.editIcon) &&
            React.createElement(Edit, { visible: over, onClick: props.onEditClick }, props.editIcon),
        (props.onDeleteClick && props.deleteIcon) &&
            React.createElement(Delete, { visible: over, onClick: props.onDeleteClick }, props.deleteIcon),
        presence &&
            React.createElement(React.Fragment, null, (presence != "offline" && presence != "invisible") &&
                React.createElement(Presence, { onClick: props.onPresenceClick || null, presence: String(presence), dark: props.dark || false })),
        props.muted &&
            React.createElement(Muted, { color: textColor, borderRadius: borderRadius }),
        React.createElement(Inner, { over: over, onClick: props.onClick, width: width, height: height, borderRadius: borderRadius, className: className, image: image, background: background, outlineInnerColor: props.outlineInnerColor ? props.outlineInnerColor : "transparent", outlineOuterColor: props.outlineOuterColor ? props.outlineOuterColor : "transparent", style: props.style },
            ...
            React.createElement("div", { style: {
                ...
                ((!props.children && !props.image && props.title && !props.onEditClick) ||
                    ...
                    React.createElement(Text, { defaultBackground: defaultBackground, color: textColor, size: props.size, background: background }, generateInitials(props.title))))));
};
